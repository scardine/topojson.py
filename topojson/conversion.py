...
def convert(inThing, outThing=None, options={}):
    if isinstance(inThing, dict):
        ...
        inFile = open(inThing)
        inpt = load(inFile)
        if not options.has_key('name') and inpt.has_key('type') and hasattr(inFile,
                                                                            'name') and inFile.name.lower().endswith(
                '.geojson'):
            inpt = {inFile.name[:-8].split('/')[-1]: inpt}
    elif isinstance(inThing, file):
        inpt = load(inThing)
        if not options.has_key('name') and inpt.has_key('type') and hasattr(inThing,
                                                                            'name') and inThing.name.lower().endswith(
                '.geojson'):
